...
-- (equipment id does not exist in equipment_rental table)
SELECT e.id, e.name 
FROM equipment e
...
-------------------------------------------
-- Query #2: employees name and surname (concatenated) that have the biggest earnings per hour
SELECT name || ' ' || surname AS employee, earnings_per_hour 
FROM employees
WHERE earnings_per_hour = (SELECT MAX(earnings_per_hour) FROM employees);

-------------------------------------------
-- Query #3: employees with numbers of contracts that they took part
...
-------------------------------------------
-- Query #6: number of archival and non-archival employees
...
-- (client id does not exist in lease_contract)
SELECT c.id, c.name 
FROM lease_contracts lc
...
-------------------------------------------
-- Query #10: programming languages with number of employees that use them 
-- (from most popular)
